# Generated by Powerlevel10k configuration wizard on 2025-07-15 at 16:55 CEST.
# Based on romkatv/powerlevel10k/config/p10k-rainbow.zsh, checksum 49619.
# Wizard options: powerline, rainbow, ascii, 24h time, 1 line, compact, fluent,
...
() {
  ...
  # Red background for main/master branches
  typeset -g POWERLEVEL9K_VCS_MAIN_BRANCH_BACKGROUND=1
  typeset -g POWERLEVEL9K_VCS_MAIN_BRANCH_FOREGROUND=7

  # Function to dynamically set VCS background based on branch
  function _p9k_vcs_set_background() {
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      if [[ $VCS_STATUS_LOCAL_BRANCH == "main" || $VCS_STATUS_LOCAL_BRANCH == "master" ]]; then
        # Set red background for main/master branches
        ...
        # Reset to default colors for other branches
  ...
  function my_git_formatter() {
    ...
    # Set dynamic background colors based on branch
    _p9k_vcs_set_background
    ...
    local   main_branch='%1F' # red foreground for main branch
    ...
    if [[ -n $VCS_STATUS_REMOTE_URL ]]; then
      ...
      if [[ $VCS_STATUS_REMOTE_URL =~ '([^/]+/[^/]+)(\.git)?$' ]]; then
        repo_name=${match[1]}
      elif [[ $VCS_STATUS_REMOTE_URL =~ '([^/]+)\.git$' ]]; then
        repo_name=${match[1]}
      elif [[ $VCS_STATUS_REMOTE_URL =~ '([^/:]+)/([^/]+)$' ]]; then
        repo_name="${match[1]}/${match[2]}"
      fi
      
      if [[ -n $repo_name ]]; then
        res+="${meta}[${clean}${repo_name//\%/%%}${meta}] "
      fi
    fi
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      # Use red color for main/master branches
      local branch_color=$clean
      if [[ $branch == "main" || $branch == "master" ]]; then
        branch_color=$main_branch
      fi
